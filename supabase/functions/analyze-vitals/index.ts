...
serve(async (req) => {
  ...
  try {
    ...
    if (!vitals || !Array.isArray(vitals) || vitals.length === 0) {
      return new Response(
        JSON.stringify({ error: "Vitals data is required" }),
        {
          status: 400,
    ...
    const HealthTrack_API_KEY = Deno.env.get("HealthTrack_API_KEY");
    if (!HealthTrack_API_KEY)
      throw new Error("HealthTrack_API_KEY is not configured");
...
Provide a comprehensive health analysis with predictions.`;
    ...
    const response = await fetch(
      "https://ai.gateway.HealthTrack.dev/v1/chat/completions",
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${HealthTrack_API_KEY}`,
