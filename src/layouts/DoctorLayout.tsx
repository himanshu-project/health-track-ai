...
import {
  ...
  User, LogOut, Heart, Menu, X, Bell, Stethoscope
} from "lucide-react";
...
export default function DoctorLayout({ children }: { children: React.ReactNode }) {
  ...
  const pageTitle = navItems.find((n) => location.pathname.startsWith(n.to))?.label || "HealthTrack";
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-sidebar-border">
          ...
          <div>
            ...
            <p className="text-sidebar-foreground/50 text-xs">Doctor Portal</p>
          </div>
          <button
        ...
        <div className="px-4 py-4 border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sidebar-foreground/50 text-xs truncate">{user?.specialization || "Doctor"}</p>
            </div>
          </div>
        </div>
        ...
        <header className="bg-card border-b border-border px-4 lg:px-6 py-4 flex items-center gap-4 shrink-0">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button variant="ghost" size="icon" className="relative">
              ...
              <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-caution" />
            </Button>
          </div>
        </header>
